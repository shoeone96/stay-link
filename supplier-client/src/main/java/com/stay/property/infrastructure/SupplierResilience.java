...
public final class SupplierResilience {
    ...
    /**
     * 이 데코레이터가 시도 하나에 허용하는 시간. 값을 밖으로 내는 이유는 하나뿐이다 — 커넥션 풀의 대기
     * 타임아웃이 <b>이 값보다 짧아야</b> 풀 고갈이 시도별 상한에 먼저 잘리지 않고 자기 예외로 드러난다
     * (D-F9-6). 서킷 상태와 달리 호출 경로가 보고 분기할 값이 아니라 배선이 읽는 값이라 패키지 안에만
     * 연다.
     */
