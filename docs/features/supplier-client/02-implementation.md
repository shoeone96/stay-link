...
| T-NN | 테스트 (클래스#메서드) | Red | Green | 비고 |
|---|---|---|---|---|
| — | (새 테스트 없음) | — | ✅ 기존 63건 전부 통과 | #1·#3 은 돌려주는 값·예외를 바꾸지 않고 로그만 더한다. 01 §5.1 테스트 리스트에 해당 T-NN 이 없어(TDD-1) 리스트를 늘리지 않았고, 대신 임시 프로브로 로그 원문을 실측한 뒤 프로브를 삭제했다(아래) |
...
- 없음. 아래 두 건은 `01-design.md` 의 문언과 달라진 자리였고, 같은 커밋에서 01 §3.4·D-F3-9 로 갱신됐다.
...
- `01-design.md` §3.4 의 분류표 3행과 번역기 규칙 문언은 이번 동작(ERROR 로그·집계 warn)에 맞춰 01 §3.4·D-F3-9 로 갱신됨.
