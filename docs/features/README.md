...
| F5 | `supplier-availability-adapter` | 사전작업 4 (재고·요금) | 구현중 | 2026-09-07 | - |
...
- **포함**
  ...
  - **k6 시나리오에 재고·요금 호출 포함** — 실제 소켓을 여는 자동 테스트를 두지 않기로 했으므로(F3 D-F3-5, F5 설계 §1 제외 표) **직렬화 표기 회귀를 잡는 장치가 k6 밖에 없다.** F5 구현 중 `LocalDate` 쿼리 파라미터가 JVM 로케일 표기(`checkIn=26. 9. 10.`)로 나가 모의 서버가 400 으로 거절한 결함이 실측에서만 드러났고, 이 갈래는 프록시를 목으로 대체하는 현재 테스트 153 개 중 어디에서도 실행되지 않는다. 목록뿐 아니라 **재고·요금 호출도 k6 로 태워** 계약 §1 의 `YYYY-MM-DD` 가 실제로 나가는지 확인한다
