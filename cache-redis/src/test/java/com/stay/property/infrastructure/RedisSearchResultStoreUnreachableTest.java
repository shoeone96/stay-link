...
class RedisSearchResultStoreUnreachableTest {
    ...
    @Test
    @DisplayName("저장소에 닿지 못하면 find 는 연산·키·원인을 담은 저장소 불가 예외를 던진다")
    ...
    @Test
    @DisplayName("저장소에 닿지 못해도 store 는 예외를 던지지 않는다")
    void store_storeUnreachable_doesNotThrow() {
        // given
        // when
        // then
        assertThatCode(() -> store.store(COMMAND, StaySearchResultFixture.partiallyFailed())).doesNotThrowAnyException();
    }
