...
2. **차단이 이득인 이유는 재시도를 막아서가 아니라 첫 호출을 막아서다.** 죽은 공급사로 나가는 호출
   하나가 시도별 상한(1.85초) 동안 커넥션 풀 슬롯을 잡고, 그 슬롯은 살아 있는 공급사가 써야 할
   것이다 (D-F9-5·6). §4.1의 "왜 토큰 버킷이 아니라 서킷인가"가 여기서 나온다.
...
| 커넥션 풀 슬롯 점유 | 검색마다 시도별 상한 × 묶음 수 | **0** |
| 사용자 체감 | 죽은 공급사 몫이 매 검색마다 소모된다 | 즉시 실패 값이 되고 살아 있는 공급사만 기다린다 |

D-F9-5·6 에서 동시 상한을 없애고 **실제 상한을 커넥션 풀 하나로 옮겼기 때문에, 그 슬롯이 이 시스템에서
가장 희소한 자원**이다. 공급사 A 가 죽어 있는데 검색이 계속 들어오면 A 로 향하는 호출들이 각각 시도별
상한 동안 슬롯을 붙들고, **그 슬롯은 살아 있는 B 가 써야 할 것이다.** 토큰 버킷은 이 점유를 한 건도
줄이지 못한다 — 첫 호출은 재시도가 아니기 때문이다.

> **결론.** 토큰 버킷은 **재시도가 만드는 추가 부하**를 줄이는 장치이고, 서킷은 **첫 호출이 자원을
...
| T-03 | supplier-client | Normal | `SupplierFailurePolicy.isRetryable` — 8개 유형 전부 (Parameterized 8) | Decision Table | §3.4 표와 일치 (O 3개 · X 5개) |
| T-04 | supplier-client | Normal | `SupplierFailurePolicy.isCircuitFailure` — 8개 유형 전부 (Parameterized 8) | Decision Table | §3.4 표와 일치 (O 4개 · X 4개) |
...
- 실제 소켓·커넥션 풀 고갈 — k6와 모의 서버로 확인하며 단위 테스트로 만들지 않는다.
...
| D-F9-6 | 그러면 실제 상한은 어디에 있나 | flatMap 기본값(256)에 맡김 / 커넥션 풀 기본값에 맡김 / **커넥션 풀 명시 설정** | **명시 설정.** 크기와 대기 타임아웃(`< per-call`)을 준다 | 기본값에 맡김: 풀 기본이 `max(코어 수, 8) × 2` 라 **배포 머신마다 상한이 달라진다.** 그리고 대기 타임아웃 기본이 45초라 우리 `per-call` 이 **먼저** 터져 `TIMEOUT` 으로 기록되고, **자사 병목이 공급사 장애로 오분류되어 서킷이 멀쩡한 공급사를 차단**한다. 대기 타임아웃을 `per-call` 보다 짧게 두면 풀 고갈이 고유한 예외로 먼저 터져 구분된다 | 예 |
