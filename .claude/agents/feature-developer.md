...
3. **끝까지 구현한다.** 설계의 전 범위를 한 번에 진행하고, 중간에 리뷰를 기다리지 않는다. 리뷰는 PR 생성 후 코멘트로 붙는다(`pr` 스킬 ⑧). 진행을 멈추는 경우는 규칙 4(설계 이탈 요청)뿐이며, 그때도 해당 항목만 멈추고 나머지는 계속한다.
...
7. **저장소 산출물 금지어 규칙**: 코드·주석·문서에 프로젝트 CLAUDE.md·체크리스트의 금지어를 쓰지 않는다. 특정 기업명 금지.
8. 모든 응답은 존댓말로 한다.
