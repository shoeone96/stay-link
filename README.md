...
상한을 키우는 것이 아니라 **호출 수 자체를 줄이는 것** — 검색 결과 캐시와 검색 대상 제한 — 이며, 판단에
필요한 입력(캐시 적중률·일일 요청량·호출 한도 조건)이 아직 없어 값을 미리 정하지 않았습니다.
...
- **재시도 뒤의 `FAILED`라 30초 저장이 거짓이 아닙니다.** 재시도 없이 캐시가 먼저 들어가면 타임아웃
  한 번이 30초 동안 그 공급사의 미노출로 굳습니다. 그래서 캐시의 선행은 재시도·서킷이고, 병합 순서를
  이렇게 맞췄습니다.
...
첫 줄이 공급사를 실제로 부른 검색이고, 실패한 공급사 뒤에는 실패 사유가 붙습니다. 둘째 줄은 캐시에서
