...
[F3a webclient-config] ───→ [F3 supplier-client] ┤                          │
                                     ...
                                     └─→ [F5 availability-adapter] → [F7 stay-search-api + F8 partial-failure]
                                                                  ...
                                                                  ┌────────┴─────────┐
                                                                  ▼                  ▼
                                                     [F9 supplier-resilience] [F10 search-cache]
                                                                  │
...
- **포함**
  ...
  - ~~retry가 예산 안에 들어오도록 계층 정합. 재시도가 붙으면 부등식이 `× (1 + 최대 재시도)`로 커진다~~ → **정정 (D-F9-3·5, 2026-09-07).** `maxConcurrent`는 삭제됐고, 재시도는 `perCall` **안에서** 도는 2단 상한이라 우변에 곱해지지 않는다. 시도별 상한이 `(perCall − 백오프 합) ÷ 시도 수`로 유도되어 "재시도를 포함한 총 소요 ≤ perCall"이 구조적으로 성립한다. 실측으로 확인됨 — 2시도 + 백오프 = 3.94s < perCall 4s
  - (선택) rate limiter — 공급사 한도 초과 유형이 관측될 때만
- **제외**: 캐시(F10)
- **선행**: F8
- **닫아야 할 결정**
  - ~~수단: Resilience4j vs Reactor `retryWhen`~~ → **닫힘 (D-F9-1·2).** Resilience4j 코어 모듈 직접 배선, 재시도도 Resilience4j
  - ~~수치(횟수·백오프·차단 임계) 와 근거~~ → **닫힘 (D-F9-3·13, §3.7).** 시도 2회 · 백오프 200ms · 지터 0.5 · 창 10 / 최소 5 / 임계 50% / 열림 60s / 탐침 2
  - ~~**`retryWhen`을 `timeout(perCall)` 안에 두는가 밖에 두는가**~~ → **닫힘 (D-F9-3·4).** 2단 상한 — 시도별 상한은 어댑터가 안쪽에, `per-call`은 조합기의 전체 상한으로 유지. 중첩 순서는 Resilience4j 공식 순서인 `Retry ( CircuitBreaker ( … ) )`
  - ~~**`per-call`·`budget`·`max-concurrent`·재시도 횟수를 한 부등식으로 함께 잡는다**~~ → **닫힘 (D-F9-5·6, 2026-09-07).** 여기 적었던 "숙소 120곳 → 호출 6건 → 이미 깨진다"는 **출처 없는 가정 위의 계산**이었다(계약·시드·요구사항 어디에도 120이 없고 `per-call`도 2s가 아니라 4s다). 실제 시드는 A 2곳·B 1곳이라 묶음이 각 1, 호출 2건이고 부등식은 성립한다. F9는 `max-concurrent`를 **프로퍼티째 삭제**해 웨이브를 항상 1로 만들었고, 남는 부등식은 `budget > per-call` 하나다. 실제 상한은 커넥션 풀로 옮겼다
