...
[F3a webclient-config] ───→ [F3 supplier-client] ┤                          │
                                     │           └─→ [F4 catalog-adapter]───┤
                                     │                                      │
                                     └─→ [F5 availability-adapter] → [F7 stay-search-api + F8 partial-failure]
                                                                           │
                                                                           ▼
                                                              [F9 supplier-resilience]
                                                                           │
                                                                  ┌────────┴─────────┐
                                                                  ▼                  ▼
...
| F10 | `search-cache` | 조회 5 | PR | 2026-09-07 | - |
...
## F7. `stay-search-api` — 통합 검색 API + aggregator (F8 흡수)

> **설계 확정 (2026-09-07).** 협의용 시각화는 `docs/features/stay-search-api/design.html`(그림 12장),
> 결정의 원본은 `01-design.md`(결정 카드 15 + 이연 해소 1, 테스트 리스트 T-01~T-19).

- **목적**: 첫 end-to-end. `GET /api/v1/stays/search` 요청 1건 → 매핑 조회 → 공급사별 코드 묶음(≤50) → 병렬 fan-out → 정규화 → aggregate → 응답. **결과가 잘렸다는 사실까지 응답에 담는다.**
- **F8을 흡수한 이유 (2026-09-07)**: F8 6개 항목 중 넷은 이미 F5(실패를 값으로 반환)·F7(응답 계약)로 넘어와 있었고 실제로 남은 것은 「전원 실패 시 응답」과 「로그·지표」 둘뿐이었다. 나눠 두면 **F7 구현이 전원 실패 경로에서 무엇을 해야 할지 모르는 채 코드를 쓰게 된다.**
- **포함**
  - 자사 API 스펙 확정 (조회 작업 1): 요청 파라미터(checkIn·checkOut·adults·children) 넷뿐, 검증 규칙(날짜 순서·과거 날짜·인원 하한), 응답 = `results[]` + `suppliers[]`
  - 매핑 역조회로 내부 `propertyId`·`roomId` 부여, 미매핑 코드는 동기 경로에서 항목 제외 + 로그 (D11 동기 트랙). 조회 메서드는 F1에서 이관된 항목으로 여기서 추가한다 (D-F1-8) — **ACTIVE만 거른다**
  - 공급사별 병렬 호출 — **F5의 포트를 호출한다.** 유스케이스는 Reactor 연산자를 직접 쓰지 않는다(`core`에 리액티브 타입이 들어오면 안 된다). 병렬·타임아웃·예산은 포트 구현이 F3a의 조합기로 처리한다
  - **부분 실패 표기(구 F8)** — `suppliers[]`에 `{supplier, status}`. 부분 실패는 200, **전원 실패는 502 `ALL_SUPPLIERS_FAILED`**
  - **검색 1건 = 요약 로그 1줄**과 레벨 승격 규칙. 실패 사유는 응답이 아니라 이 줄에 남고 지표의 재료가 된다
  - API 문서 자동화(테스트가 문서를 만든다)와 루트 `README.md` 신설, `k6/app-search.js` 채우기와 타임아웃 값 실측
- **제외**: retry/circuit(F9), 캐시(F10), 미매핑 비동기 수정(F11), 공급사 간 숙소 병합(D5), 페이지네이션·정렬 파라미터, fan-out 세 값 재산정(F9), 지표 수집기 도입(sink가 없다)
- **선행**: F1, F5, **F6**(lifecycle·매핑 데이터). `PropertyLifecycle`이 없으면 ACTIVE 필터가 컴파일되지 않는다
- **닫힌 결정** (2026-09-07 설계에서 확정)
  - ~~요청 검증 실패 응답 형식~~ — **F0에서 확정**. `ApiResponse` 형식과 검증 실패 처리를 그대로 쓴다
  - ~~결과 정렬 기준~~ — **숙소명 → 객실명 → 공급사 고정** (D-F7-1). 가격 오름차순은 조식 조건이 다른 항목을 같은 축에 세워 탈락
  - ~~코드 묶음 분할을 지금 넣을지~~ — **F5가 어댑터에서 처리한다** (D-F5-5). F7은 포트를 호출만 한다
  - `suppliers[]`에서 `reason` 제외 — **D10 개정** (D-F7-4). 공급사 어휘가 응답 경계를 넘지 않게
  - `lifecycle` 인덱스는 **닫지 않고 관측 항목으로 유지** (D-F7-7) — INACTIVE 행이 단조 증가하므로 전제가 시간이 지나면 깨진다
- **완료 기준**: 모의 서버 정상 모드에서 검색 요청 1건에 A·B 상품이 합쳐진 응답이 오고 `suppliers[]`가 모두 OK다. B를 장애 모드로 두면 A 결과만 담긴 응답이 예산 안에 오고 B가 FAILED로 남는다. 둘 다 내리면 502와 ERROR 로그가 남는다.
...
> **범위 확정 (2026-09-07).** 착수 전 고민과 조사(출처 51건 검증)는 `docs/ai-history.md` 89·90번.
> 설계 협의는 `docs/features/search-cache/design.html`, 결정의 원본은 `01-design.md`.

- **목적**: 같은 검색 조건(날짜·인원 넷)으로 30초 안에 다시 들어오는 요청이 공급사를 부르지 않게 한다. 공급사 한도는 계정 단위라 `api-app` 인스턴스가 늘수록 같은 조건의 호출이 곱해지는데, 이 캐시가 그 배수를 지운다. **정확성 장치가 아니라 공급사 호출량 절감 장치**다 — 검색가는 견적이고 예약 직전 재확인이 업계 표준이라 짧은 캐시는 관행과 충돌하지 않는다.
- **포함**
  - 캐시 자리는 `SearchStaysUseCase.search()` 경계, 키는 `StaySearchCommand` 넷(checkIn·checkOut·adults·children)
  - **TTL 30초 하나**, 설정 키. 값의 근거는 정확성이 아니라 절감 배수이며 실측 후 재조정한다
  - 전원 OK·부분 실패(PARTIAL·FAILED 섞임) 응답 모두 **그대로** 30초 저장 — 재시도(F9)를 거친 실패는 그 시점의 사실이다
  - **전원 실패도 30초 기억한다**(negative cache) — 기억이 살아 있는 동안 공급사를 부르지 않고 즉시 502. 한 인스턴스가 겪은 전원 실패가 Redis 를 통해 나머지 인스턴스를 바로 보호한다
  - single-flight — 같은 키의 동시 miss 는 JVM 안에서 한 번만 부른다. 분산 락은 두지 않으므로 만료 순간 최대 인스턴스 수만큼 호출이 나간다
  - 저장소는 Redis(scale-out 전제) + JSON 직렬화. **Redis 에 못 닿으면 503**(`SEARCH_UNAVAILABLE`, "지금은 매물을 확인할 수 없다") — 캐시는 공급사 한도 안에 머무르기 위한 장치라 우회로 트래픽을 흘리지 않는다(D-F10-4, 설계 중 우회안에서 뒤집힘). 끊긴 동안 즉시 거절 + 명령 타임아웃 300ms 로 503 이 바로 나간다. 공급사를 이미 부른 뒤의 저장 실패는 결과 반환 + WARN
  - `@Cacheable` 이 아니라 **캐시 컴포넌트를 직접 쓴다** — 컴포넌트와 포트 `SearchResultStore` 는 `core` application, Redis 구현·설정·Testcontainers 는 **새 모듈 `cache-redis`**(persistence·supplier-client 와 같은 자리, api-app 은 runtimeOnly). `batch-app` 은 그 모듈을 의존하지 않고 대역 빈 한 줄로 뜬다 (D-F10-2·5)
  - `compose.yaml` 에 redis 서비스 추가. 캐시 왕복·TTL 검증에 Testcontainers Redis 1개(첫 컨테이너 의존)
- **제외**: soft/hard 이중 TTL·비동기 갱신 · 마지막 정상 값 fallback(F9 가 넘긴 항목 — 3분 전 값을 내는 것이 "판매 불가 상품을 판매 가능처럼 노출"이라 도메인이 반대) · 응답의 견적 시각(`quotedAt`) · 목록 대표 가격 · 분산 락 · 로컬 캐시(Caffeine) · 503 + `Retry-After` · rate limiter(F9 가 넘긴 항목 — 한도 초과가 여전히 관측되지 않고, 캐시가 먼저 호출 수를 줄인다)
- **선행**: **F9** (F8 에서 변경). 재시도 없이 캐시가 먼저 들어가면 1회 타임아웃이 30초 미노출로 증폭된다 — 도메인이 반대한 바로 그 경우다
- **닫힌 결정** (2026-09-07 착수 전 확정, 근거는 ai-history 89·90번)
  - ~~수단: 로컬 캐시 vs 기존 의존성~~ — **Redis.** 공급사 한도가 계정 단위라 로컬 캐시는 인스턴스 수만큼 호출을 곱하고, 롤링 배포마다 비어 공급사에 몰린다
  - ~~TTL 값과 근거, 부분 실패 응답 캐시 정책~~ — **30초 하나 · 부분 실패도 그대로 · 전원 실패는 기억.** 캐시 허용 수치의 공개 기준은 없고 실시간 값의 캐시를 금하는 공급사도 있어, 30초는 프로젝트 판단이며 설정 키로 뺀다
  - `@Cacheable(sync = true)` 탈락 — Redis 백엔드는 키 단위 single-flight 가 없고(잠금은 캐시 이름 단위), 예외는 저장되지 않아 전원 실패를 남길 수 없으며, 오류 우회 경로에서 유스케이스 예외가 Redis 예외로 바뀐다
- **재검토 조건**: 공급사 노출 누락 이의가 실제로 오면 캐시 단위를 (검색, 공급사)로 쪼개 실패 공급사만 다시 부른다 · 클라이언트가 `Retry-After` 를 읽게 되면 기억된 전원 실패를 503 으로 바꾼다 · 인스턴스 수 × miss 가 공급사 한도에 닿으면 분산 락을 본다
- **완료 기준**: 동일 조건 N개 동시 요청에 공급사 호출이 1회다. 30초 안의 두 번째 요청은 공급사를 부르지 않는다. 두 공급사를 모두 내린 뒤 30초 안의 요청은 호출 없이 502 가 온다. Redis 를 내리면 공급사 호출 없이 503 이 300ms 안에 온다. F7 의 E2E 가 무변경으로 통과한다.
- **구현 순서 (2026-09-07 사용자 결정)**: F9 가 다른 세션에서 구현 중이라 기다리지 않고 시작한다. F9 병합 후 그 `main` 을 이 브랜치에 병합해 재검토한다 (D-F10-15). 코드 충돌은 없다 — F9 는 `core` 무변경, F10 은 `supplier-client` 무변경.

## F11. `unmapped-code-recovery` — 미매핑 코드 비동기 수정 (선택)

> **하지 않는다 (2026-09-07 결정).** 검색 응답에 나오지 않는 코드를 검색 경로에서 정리(비활성·목록 재조회)하는 안을 검토했으나 접었다.
> 검색 조건이 날짜·인원뿐이고 개별 숙소 상세 조회 경로가 없어 검색 결과가 노출의 전부다. 응답에 없는 항목은 그 검색에서만
> 빠지면 되고(D11 동기 경로, F7), `property`·`room` 갱신은 하루 1회 배치(F6)의 전체 대조로만 한다. 아래는 검토 당시의 범위 기록이다.
