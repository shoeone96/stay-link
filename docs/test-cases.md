...
| T-02 | `FanOutExecutorTest#runAll_withAnyNumberOfCalls_subscribesThemAllInOneWave` (Parameterized 2) | supplier-client | 호출 2건 / 3건 → 최대 동시 구독 수가 각각 정확히 2·3 | ✅ | 높음 — 바깥 호출의 동시 수를 지키는 유일한 테스트. **F9(2026-09-07)에서 이름·기대값이 바뀌었다** — 동시 상한 프로퍼티가 사라지고 요청마다 호출 수로 잡히면서(D-F9-5) "상한을 넘지 않는다"가 "호출 수와 같다"가 됐다. 여전히 `concatMap` 치환 변이에 실패한다 |
...
| T-08 | `FanOutPropertiesTest#bind_withInconsistentValues_failsAtStartup` (Parameterized 2) | supplier-client | `budget == per-call` / `budget < per-call` → 컨텍스트 기동 실패, 실패 메시지에 어긋난 **키 이름**이 들어 있다 | ✅ | 높음 — 예산 부등식을 요청 시점이 아니라 기동 시점으로 끌어온다. 키 이름까지 보므로 "아무 이유로든 실패"로는 통과하지 않는다. **F9 에서 `max-concurrent = 0` 행이 빠졌다** — 그 프로퍼티가 사라졌고, 대신 이 부등식이 최소 조건이 아니라 완전한 조건이 됐다(D-F9-5) |
...
| T-17 | `CatalogFanOutPropertiesTest#bind_withInconsistentValues_failsAtStartup` (Parameterized 2) | supplier-client | `budget == per-call` / `budget < per-call` → 기동 실패, 메시지에 `supplier.catalog.fan-out.*` 키 | ✅ | 높음 — 수용 기준 5 의 "수집 정책도 부등식 검사를 받는다". 키 이름을 보므로 검색용 검사로 통과하지 않는다. **F9 에서 `max-concurrent = 0` 행이 빠졌다**(D-F9-5) |
...
| T-08 | `SupplierResilienceTest#decorate_betweenAttempts_waitsWithinJitterRange` | supplier-client | 가상 시간에서 최초 대기 100ms · 지터 0.5 → 50ms 동안 아무 신호 없음, 이후 100ms 안에 재시도 성공 | ✅ | 높음 — 백오프가 실제로 걸리는지를 시계에 묶이지 않고 본다. `intervalFunction` 을 빼면 즉시 재시도가 되어 `expectNoEvent` 가 실패한다. Red 없이 통과 |
| T-09 | `SupplierResilienceTest#decorate_whenFailuresAreBelowMinimumCalls_keepsCallingSupplier` | supplier-client | 최소 호출 수 5 · 시도 1회에서 연속 실패 5건 → 다섯 번 모두 공급사까지 도달 | ✅ | 높음 — 설계 §3.7 전 과정표의 호출 1–4 구간. **변이 검사 A**(`minimumNumberOfCalls(1)`)에서 실패해 "우연 몇 건에 열리지 않는다"를 실제로 지키는 것을 확인했다. Red 없이 통과 |
| T-09 | `SupplierResilienceTest#decorate_whenFailureRateReachesThreshold_opensAndStopsCallingSupplier` | supplier-client | 표본 5건이 전부 실패한 뒤 다음 호출 → `CallNotPermittedException`, 구독 수가 5 에서 늘지 않음 | ✅ | 높음 — 수용 기준 4. "호출이 나가지 않는다"를 예외 타입이 아니라 **구독 수**로 보므로, 예외만 흉내 내고 호출은 나가는 구현으로는 통과하지 않는다. Red 는 서킷 배선 전의 실제 실패 |
| T-10 | `SupplierResilienceTest#decorate_afterOpenStateExpires_permitsOnlyTheConfiguredProbeCount` | supplier-client | 열림 대기 경과 후 동시 3건 → 앞 2건만 공급사로 나가고 3건째는 `CallNotPermittedException`, 그 거부가 탐침 지연(300ms)보다 빠름 | ✅ | 높음 — half-open 의 거부가 줄 세우기가 아니라 즉시 거부라는 것까지 본다. 전이를 일으키는 것이 타이머가 아니라 **호출**이라는 것도 여기서 드러난다(테스트가 직접 호출자 역할을 한다). **변이 검사 B**(허용 수 3)에서 실패 |
| T-11 | `SupplierResilienceTest#decorate_afterHalfOpenProbes_closesOrReopens` (Parameterized 2) | supplier-client | 탐침 2건 모두 성공 → 세 번째 호출도 공급사 도달(구독 3) / 탐침 2번이 실패 → 세 번째는 차단(구독 2) | ✅ | 높음 — AWS 가 "테스트하기 어렵다"고 지목한 모달 동작의 양쪽 전이(설계 §4.1 대응 2). 허용 수 2 + 임계 50% 조합이 "하나라도 실패하면 되돌린다"로 도는지가 핵심이고, **변이 검사 B**(허용 수 3)에서 실패 케이스가 닫혀 버려 잡혔다 |
| T-12 | `SupplierResilienceTest#decorate_whenCircuitIsOpen_doesNotRetryTheBlockedCall` | supplier-client | 시도 2회 정책으로 서킷을 연 뒤 호출 1건 → 차단 계측이 정확히 1 증가 | ✅ | 높음 — 재시도가 서킷 바깥이라는 순서(D-F9-4)의 필수 짝. 재시도했다면 다시 구독하며 2가 된다. **변이 검사 C**(`CIRCUIT_OPEN` 을 재시도 대상에 넣고 `ignoreExceptions` 제거)에서 실패 |
| T-13 | `SupplierResilienceTest#decorate_whenOneSupplierCircuitIsOpen_leavesTheOtherSupplierUntouched` | supplier-client | A 의 서킷을 연 뒤 B 호출 → 성공, B 구독 1회 | ✅ | 높음 — 수용 기준 6(공급사별 격리). **변이 검사 D**(레지스트리 키에서 공급사를 빼 한 벌만 만들기)에서 이 테스트만 실패했다 |
| T-14 | `FailureClassifierTest#classify_callNotPermitted_mapsToCircuitOpen` | supplier-client | `CallNotPermittedException` → `CIRCUIT_OPEN` | ✅ | 높음 — D-F9-8. `UNAVAILABLE` 로 재사용하면 F7 의 요약 로그가 자사 차단을 공급사 장애로 적어 그 줄이 거짓 문장이 된다. Red 는 상수 부재로 인한 컴파일 실패 |
| T-15 | `FailureClassifierTest#classifyQuietly_unmappedException_mapsToUnexpectedWithoutErrorLog` | supplier-client | 분류표에 없는 예외를 판정용 진입점으로 → `UNEXPECTED` 이고 수집한 로그에 ERROR 없음 | ✅ | 높음 — D-F9-11. 판정 진입점을 다시 `classify` 로 되돌리면 재시도·서킷·어댑터가 각각 불러 ERROR 가 여러 줄 찍히고, 이 테스트가 먼저 실패한다. Red 는 메서드 부재 |
| T-16 | `FanOutExecutorTest#runAll_withAnyNumberOfCalls_subscribesThemAllInOneWave` (Parameterized 2) | supplier-client | 호출 2건 / 3건 → 최대 동시 구독 수가 정확히 2·3 | ✅ | 높음 — D-F9-5 의 "웨이브가 항상 1". 이 성질이 곧 부등식이 `budget > per-call` 하나로 줄어드는 근거라, 여기가 깨지면 기동 시 검사도 함께 거짓이 된다. Red 는 생성자 시그니처 변경에 따른 컴파일 실패 |
| T-17 | `FanOutExecutorTest#runAll_withNoCalls_returnsEmptyList` | supplier-client | 호출 목록이 빔 → 빈 리스트 | ✅ | 높음(F3a 의 중간에서 승격) — 동시성을 `calls.size()` 로 잡은 뒤로 `Math.max(1, ...)` 가드를 지키는 자리가 됐다. 가드를 빼면 `flatMap` 이 동시성 0 을 거부해 즉시 터진다 |
| T-18 | `SupplierResiliencePropertiesTest#bind_withOutOfRangeValue_failsAtStartup` (Parameterized 9) | supplier-client | 아홉 키를 하나씩 범위 밖 값으로 → 기동 실패, 메시지에 `supplier.resilience.<키>` | ✅ | 높음 — 틀린 값의 증상이 "가끔 이상하다"로만 나타나는 종류라 기동 시점으로 끌어온다. 특히 `minimum-number-of-calls > sliding-window-size` 는 값 하나하나는 멀쩡하고 조합만 틀린 경우이고, 통과하면 서킷이 영영 열리지 않는다 |
...
요약: 총 26 · 통과 26 · 실패 0 · 건너뜀 0 (기능 테스트만. 저장소 전체는 총 237 · 통과 237 · 실패 0 · 건너뜀 0 — F9 병합 후 총 298 · 통과 298 · 실패 0)
...
| # | 테스트 (클래스#메서드) | 레이어 | 상세 내용 | 통과여부 | 유의미함 |
|---|---|---|---|---|---|
| T-01 | `StaySearchCacheTest#getOrLoad_withoutStoredResult_loadsOnceAndStores` | application | 저장값 없음 → loader 1회 · 결과가 store 에 저장 · outcome MISS | ✅ | 높음 — Cache-Aside 의 miss 경로(D-F10-1) 전체를 고정한다. Red 는 컴파일 실패 |
| T-02 | `StaySearchCacheTest#getOrLoad_withStoredResult_returnsStoredWithoutLoading` | application | 저장값 있음 → loader 0회 · outcome HIT · 저장값 그대로 | ✅ | 높음 — 수용 기준 2("30초 안 두 번째 요청은 공급사도 DB 도 부르지 않는다")의 컴포넌트 쪽 근거. Red 없이 통과했고 **변이 검사 A**(find 결과 무시)에서 이 테스트만 실패했다 |
...
| T-09 | `SearchStaysUseCaseTest#search_allSuppliersFailedOnFetch_storesFailedResultBeforeThrowing` | application | A·B 실패 묶음만 → 예외가 나기 **전에** 전원 FAILED 결과가 store 에 저장 | ✅ | 높음 — F7 의 "판정 → throw" 를 "저장 → 판정" 으로 바꾼 자리(§3.4). `fetch` 가 던지면 저장이 건너뛰어져 negative cache 가 사라진다 |
| T-10 | `StaySearchResultTest#allSuppliersFailed_byOutcomes_isTrueOnlyWhenNonEmptyAndAllFailed` (Parameterized 3) | application | outcomes 빈 / 전부 FAILED / FAILED+PARTIAL → false / true / false | ✅ | 높음 — D-F7-15 공허참 방지가 `Collected` 에서 값 객체로 옮겨 온 자리. F7 T-20 과 같은 규칙을 값 쪽에서 고정한다. Red 는 컴파일 실패 |
...
- **Red 없이 통과한 것 8건**(T-02·T-03·T-05·T-06·T-12·T-13·T-14 의 두 메서드)은 전부 **앞선 사이클이 그 규칙을 함께 구현한 경우**다. 설계 §3.3 의 절차는 find → putIfAbsent → load → store → complete 가 한 덩어리라, T-01 이 find 와 store 를, T-04 가 예외 전파와 정리를 함께 들여왔다. 어댑터도 키·TTL·예외 변환이 메서드 둘에 같이 산다. 그래서 여덟 건에 **변이 검사 A~H** 를 붙여 각각 "그 줄을 고치면 이 테스트가 실패한다"를 확인했다(상세는 `02-implementation.md`).
- **T-07~T-09 는 Red 가 하나였다.** 유스케이스 생성자에 `StaySearchCache` 가 더해지는 순간 셋이 함께 컴파일 실패였고, 사이클 1회에 셋을 Green 으로 만들었다. 셋은 같은 변경(§3.4 의 `search` 재구성)의 세 갈래라 나누면 중간 상태가 컴파일되지 않는다.
- **T-14 는 메서드 둘이다.** 설계의 한 행이 서로 다른 두 행동(find 는 던진다 · store 는 던지지 않는다)을 적었고, 한 테스트에 몰지 않는다(TST-2).
- **T-13 은 실제로 1.2초를 잔다.** TTL 만료는 Redis 가 세는 것이라 시계를 대신 돌릴 수 없다. 이 모듈의 테스트 시간 대부분이 이 한 건과 컨테이너 기동이다.
