...
class SupplierBAvailabilityFetcherTest {
    ...
    @Mock private SupplierBApi api;
    @InjectMocks private SupplierBAvailabilityFetcher fetcher;
    ...
    void call_whenApiThrowsSynchronously_failsInsideMono() {
        ...
        given(api.search(anyString(), any(), any(), anyInt(), anyInt())).willThrow(cause);

        // when — 여기서 던지면 조합기가 받을 자리가 없다
        Mono<List<AvailabilityOffer>> mono = fetcher.call(List.of("P-88410"), QUERY);
