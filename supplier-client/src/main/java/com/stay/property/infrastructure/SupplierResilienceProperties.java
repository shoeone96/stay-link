...
/**
 * 검색용 재시도·서킷 설정. 값이 정합하지 않으면 요청이 들어온 뒤가 아니라 <b>기동 시점에</b> 실패한다.
 * 여기서 막지 않으면 증상이 늦고 모호하다 — 지터가 붙은 백오프도, 최소 호출 수를 못 채워 영영 열리지
 * 않는 서킷도 눈에 보이지 않고 "가끔 이상하다"로만 나타난다.
 *
 ...
 */
@ConfigurationProperties(prefix = SupplierResilienceProperties.PREFIX)
public record SupplierResilienceProperties(
        int maxAttempts,
        Duration minBackoff,
        Duration maxBackoff,
        double jitterFactor,
    ...
    static final String PREFIX = "supplier.resilience";
     ...
     */
    public SupplierResilienceProperties {
        try {
