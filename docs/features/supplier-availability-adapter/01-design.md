...
`453600`이 45만 원인지 $4,536.00인지 값만 보고 알 수 없다.** 이유는 이 하나로 충분하다 — 값의 뜻이
숫자만으로 정해지지 않으므로 통화가 값을 따라다녀야 한다.

> **`plus`의 통화 가드는 지금은 실행되지 않는다** (리뷰 round-1 #4). 계약의 `currency`는 **항목 단위 필드
> 하나**여서 번역기는 그 하나로 한 항목의 모든 `Money`를 만든다. 두 피연산자의 통화가 다를 경로가 없고,
> `plus`의 호출자도 A 번역기 한 곳뿐이다. 그러므로 가드는 "지금 막고 있는 계약 위반"이 아니라 **A가 날짜별
> 통화를 싣기 시작하면 그때 효력이 생기는 예약된 검사**다. T-02는 그 갈래를 미리 고정해 둔 것이며, `Money`를
> 값 객체로 둔 근거는 가드가 아니라 위 문단 하나로 성립한다.
...
> **재검토 조건 — 2026-09-07 F9에서 닫혔다.** 여기 적었던 부등식
> `budget > ⌈호출 수 ÷ max-concurrent⌉ × per-call`은 **좌변의 `max-concurrent`가 사라져** 더 이상
> 성립하지 않는다. F9가 동시 상한 프로퍼티를 삭제하고 요청마다 호출 수만큼 동시에 보내기로 해
> 웨이브가 항상 1이 되었고(D-F9-5), 남는 조건은 `budget > per-call` 하나다. "공급사당 숙소 50개까지만
> 만족한다"는 서술도 그래서 맞지 않는다. **동시 상한의 단위(전역/공급사별)** 문제는 상한 자체가
> 없어져 함께 닫혔고, 실제 상한은 커넥션 풀로 옮겼다(D-F9-6).
>
> 다만 **① 캐시 적중률 ② 일일 요청량 ③ rate limit 조건**은 여전히 확보되지 않았고 **F10의 입력**이다.
> **rate limit 값은 계약에 없으므로**(계약은 429·`E429`의 존재만 말한다) 그때 먼저 확보해야 한다.
...
| D-F5-14 | 체류 날짜를 어떤 시간대 기준으로 두나 | UTC `Instant` 로 변환 / `ZonedDateTime` / **`LocalDate` 유지** | `LocalDate` 유지, 코드 변경 0 | UTC 변환: 체크인은 시각이 아니라 달력 날짜다. `Instant` 왕복은 정밀도를 올리는 게 아니라 **하루가 밀리는 연산**을 들여온다(KST 09-10 00:00 → `2026-09-09T15:00Z`). 3박이 2박이 되고 `stayDates()` 집합이 통째로 어긋나 총액과 재고가 함께 틀린다 · `ZonedDateTime`: 시간대가 필요한 칸(과거 날짜 거부의 "오늘", 취소 기한)이 **이 범위에 하나도 없고**, 계약에 숙소 시간대 필드가 없어 계산할 입력 자체가 없다 | 아니오 |

### 6.1 글로벌 확장 시 고려 (D-F5-14 부록, 지금은 구현하지 않는다)

날짜·시각은 **역할이 셋이고 타입이 셋**이다. "글로벌 = UTC"가 아니라 **"시각은 UTC, 체류 날짜는
`LocalDate`, 둘을 잇는 재료가 숙소 `ZoneId`"** 가 정확한 명제다.

| 역할 | 타입 | 이 범위의 상태 |
|---|---|---|
| 체류 날짜 (체크인·체크아웃, 날짜별 재고·요금) | `LocalDate` | 있음 — 현행 유지 |
| 순간 (응답 타임스탬프·로그, 장차 예약 생성 시각) | `Instant` (UTC) | `ApiResponse` 가 이미 `Instant` — 맞는 자리 |
| 숙소 현지 기준을 만드는 재료 | `ZoneId` | **없음** — 계약에 숙소 시간대 필드가 없다 |

확장 시 추가되는 것은 UTC 변환이 아니라 **숙소 `ZoneId` 를 데이터로 확보하는 것**이다. "체크인 당일 현지
18:00 까지 무료 취소" 같은 값은 `LocalDate` + `LocalTime` + 숙소 `ZoneId` → `Instant` 로 만들어지고,
"오늘 이전 날짜 거부"의 그 **"오늘"도 숙소 현지 기준**이라 `ZoneId` 없이는 계산되지 않는다.

**재검토 조건** — 아래 중 하나라도 발생하면 이 카드를 다시 본다.

1. 공급사가 **숙소 시간대**를 응답에 싣기 시작한다 (그 전에는 구현할 입력이 없다)
2. **무료 취소 기한** 같은 시각 값이 계약에 들어온다 → 오프셋 있는 ISO 8601 로 받아 `Instant` 로 정규화
3. **과거 날짜 거부**를 넣는다 (F7 후보) → `LocalDate.now()` 를 직접 부르지 말고 `Clock` 을 주입한다.
   그래야 테스트가 실행 시각에 흔들리지 않고 기준 시간대가 설정에 드러난다
4. 해외 숙소가 유입된다 → D-F5-2 의 KRW 단일 통화 전제와 **같은 시점에 함께 깨지는 부채**다

> **근거의 출처.** 글로벌 OTA·GDS 공개 API 다섯 곳(Expedia Rapid · Booking.com Demand · Amadeus Hotel
> Search · Google Hotel Prices · Hotelbeds)의 조사에서 **다섯 곳 모두 체류 날짜를 시간대 없는 달력 날짜로
> 두고**, 기한 같은 시각 값만 오프셋 있는 ISO 8601 로 준다는 공통점이 확인됐다. 표시 기준은 갈린다 —
> Booking.com 은 UTC 로 정규화하고 Expedia·Amadeus·Google 은 숙소 현지 오프셋으로 준다. `LocalDate` 의
> 자바 공식 정의("오프셋이나 시간대 정보 없이는 시간선 위의 한 순간을 나타낼 수 없다")와도 같은 구분이다.
> **이 조사는 세션 안에서 URL 을 재확인하지 않았으므로 링크를 적지 않는다** — 문서에 출처를 싣기로 하면
> `tech-research` 의 검증을 거친 뒤 이 자리에 넣는다.
