...
/** 수집용 정책도 검색용과 같은 최소 조건({@code budget > per-call}, {@code max-concurrent >= 1})을 기동 시점에 받는다. */
class CatalogFanOutPropertiesTest {
    ...
    @CsvSource({
        "예산이 호출당 상한과 같으면, 2, 30s, 30s, supplier.catalog.fan-out.budget",
        "예산이 호출당 상한보다 작으면, 2, 30s, 10s, supplier.catalog.fan-out.budget",
        "동시 호출 상한이 1보다 작으면, 0, 30s, 40s, supplier.catalog.fan-out.max-concurrent"
    })
    ...
    void bind_withInconsistentValues_failsAtStartup(
        ...
        ApplicationContextRunner context =
                runner.withPropertyValues(
                        "supplier.catalog.fan-out.max-concurrent=" + maxConcurrent,
