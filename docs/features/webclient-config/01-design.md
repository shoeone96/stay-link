...
- **F9** — 재시도를 붙이면 예산 부등식이 커지고, `retryWhen`을 `timeout(perCall)` 안에
  ...
  근거가 없다.
- **F9 — 세 값을 따로 정할 수 없다.** 묶음 분할 때문에 부등식의 좌변이 공급사 수가 아니라
  **호출(묶음) 수**로 커진다. 숙소 120곳이 A·B 각 3묶음이 되면 호출 6건이고, 현재 값
  (`max-concurrent 2` · `per-call 2s` · `budget 5s`)은 **재시도를 붙이기 전에 이미 깨진다**
  (`⌈6÷2⌉ × 2s = 6s > 5s`). 재시도 2회를 얹으면 18s가 되는데, 이는 `budget`을 늘려서 감당할
  값이 아니다 — 검색은 사용자가 기다리는 동기 경로다. `max-concurrent`를 6으로 올리면
  웨이브가 3에서 1로 줄어 같은 재시도가 6s에 들어온다. 즉 **예산보다 동시 상한이 병목**이며,
  대신 공급사에 한 번에 나가는 부하와 자사 동시 요청 수의 곱을 함께 봐야 한다.
