...
- **로컬 캐시(Caffeine 등)는 고민했지만 아직 넣지 않았습니다.** Redis 를 고른 이유는 공급사 한도가 계정
  단위라 인스턴스마다 따로 캐시하면 호출이 인스턴스 수만큼 곱해지고, 배포 때마다 비어 공급사에 몰리기
  때문입니다. 그 대가로 **Redis 가 뻗으면 검색이 503 으로 서는, 서비스 쪽 장애**가 됩니다. 그래서 이
  설계에서 중요한 것은 우회 경로가 아니라 **Redis 를 잘 구성하는 것**(복제·자동 승격·연결 감시)이며,
  운영 중 그 구성이 충분히 안전하지 않다고 판단되면 **Redis 앞에 아주 짧은 로컬 캐시**(수 초, JVM 안)를
  두어 Redis 순단을 흡수하는 안을 열어 두었습니다. 짧게 두는 이유는 로컬 캐시가 길어질수록 위의
  "인스턴스 수만큼 곱해진다"가 되살아나기 때문입니다.
- **Redis 재기동은 재연결됩니다 — 단, 주소가 같을 때만.** 실기동에서 compose가 Redis를 임시 호스트 포트로
  내보내던 동안 컨테이너만 재기동하면 포트가 바뀌어 앱이 옛 주소로만 재연결하며 503에 머무는 것을
  확인했고, `compose.yaml`의 호스트 포트를 `6379`로 고정해 닫았습니다. 실서비스에서도 같은 전제 —
  Redis 주소는 고정이어야 합니다.
