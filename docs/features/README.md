...
|---|---|---|---|---|---|
| F0 | `api-response` | 조회 1 파생(자사 API 응답·오류 본문) | 구현 대기 | 2026-09-04 | - |
...
- **포함**
  ...
  - `ErrorCode` 인터페이스 + 자사용 구현 enum, 오류 유형 `ErrorType` 4종. HTTP 매핑은 advice가 한다 (D-F0-3)
  - `BusinessException`과 `@RestControllerAdvice`. 기존 `InvalidMappingException`이 이 루트를 상속한다
  - 요청 검증(`@Valid`) 실패 처리 — 위반 필드를 `message`에 싣고 `data`는 null (D-F0-8)
  - 미분류 예외를 내부 오류로 변환 + error 로그 (D-F0-5)
  - `coding-standard` LAY-6에 횡단 요소(cross-cutting concern) 예외 조항 추가
- **제외**: 예외 처리 필터와 응답 Writer(F7 — 지금은 잡을 예외가 0개, D-F0-4), 없는 경로의 본문 통일(F7, D-F0-7), 공급사 실패 유형(D12), 필드별 오류 목록
- **선행**: 없음. F1과 독립이며 F7보다 앞선다
- **닫아야 할 결정**: 없음. D-F0-1 ~ D-F0-10 전부 닫힘
- **완료 기준**: 테스트 전용 컨트롤러로 성공·비즈니스 예외·검증 실패·미분류 예외 네 경로가 모두 같은 본문 구조를 내고, 예외 원본 메시지가 응답에 실리지 않는다.
