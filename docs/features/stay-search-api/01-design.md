...
공급사당 수십 개가 된다. F3a가 남긴 부등식 `budget > ⌈호출 수 ÷ max-concurrent⌉ × per-call`은 F9가
`max-concurrent`를 삭제하면서 `budget > per-call` 하나로 줄었지만(D-F9-5, 2026-09-07 정정), 호출이
수십 건이 되면 그 하나도 지킬 수 없다 — 웨이브가 1이어도 가장 느린 호출이 예산을 넘긴다.
...
| D-F7-16 | 공통 advice가 컨텍스트 예외를 아는 것 (리뷰 round-1 반론, **이연**) | 지금 고친다 / **이연** | 이연 — 두 번째 컨텍스트의 오류 코드가 생길 때 D-F0-3과 함께 다시 본다 | `common.web.GlobalExceptionHandler`가 `property.application.AllSuppliersFailedException`을 import한다. LAY-1의 의존 방향은 어기지 않지만 **LAY-6이 `common`을 "특정 context에 속하지 않는 것"으로 정의한 방향과 어긋난다** — context가 늘 때마다 공통 advice에 그 context의 핸들러가 하나씩 붙는다. 대안은 `BusinessException`이 상태(또는 상태로 옮길 수 있는 값)를 들고 advice가 타입을 모른 채 매핑하는 것이며 F0의 "예외 타입 = 오류 유형"을 깨지 않는다. **지금 고치지 않는 이유**: F0이 세운 예외 구조를 바꾸는 일이라 F7 범위를 넘고, 컨텍스트가 하나뿐인 지금은 비용이 관측되지 않는다. **발동 조건**: 두 번째 컨텍스트가 자기 오류 코드를 갖는 시점. **사례 추가 (2026-09-07, F10)**: `SearchCacheUnavailableException` 의 503 핸들러가 같은 advice 에 붙어 컨텍스트 예외 import 가 둘이 됐다 — 여전히 컨텍스트는 하나라 발동 조건은 미충족 | 아니오 |
...
- ~~`api-docs/index.html`(스펙을 읽어 렌더링하는 로더)만 커밋하고 **생성물 `openapi3.json`은 `.gitignore`** 한다.~~ **정정(2026-09-07, PR #12 리뷰)**: 저장소를 읽는 사람이 빌드 없이 문서를 볼 수 있어야 하므로 `openapi3.json` 도 커밋한다. 생성물이라는 사실은 `copyApiSpec` 태스크가 매번 덮어쓰는 것으로 보장한다.
